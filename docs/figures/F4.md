...
cgp_RRBS.noNormals_class_probabilitiesFig.reformatted <- ggplot() +
  ...
  scale_y_discrete(limits = rev(class_probabilities_limits), labels = rev(class_probabilities_labels)) +
...
```
# Configure annotation data
cgp_RRBS.noNormals.updated_purity <- data.frame(Sample=cgp_RRBS.noNormals_sample_info$Tumor_Sample_Barcode,Purity=cgp_RRBS.noNormals_sample_info$purity)
cgp_RRBS.mutations <- data.frame(Sample=cgp_RRBS.noNormals_sample_info$Tumor_Sample_Barcode,Total_MutRate=cgp_RRBS.noNormals_sample_info$adusted_coding_mutrate)
cgp_RRBS.age <- data.frame(Sample=cgp_RRBS.noNormals_sample_info$Tumor_Sample_Barcode,Age=cgp_RRBS.noNormals_sample_info$age_updated)
...
mutationsFig <- ggplot(data = cgp_RRBS.noNormals_sample_info, mapping = aes(x = Tumor_Sample_Barcode, y = 1, fill = adusted_coding_mutrate)) + 
  ...
  ```
  cgp_RRBS.noNormals_class_probabilitiesFig.reformatted.combinedVer <- cgp_RRBS.noNormals_class_probabilitiesFig.reformatted + theme(axis.title.x=element_blank(),
                                                                                                                             axis.text.x=element_blank(),
                                                                                                                             axis.ticks.x=element_blank())
